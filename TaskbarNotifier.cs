...
namespace StreamingNotificationsSample
{
	...
	public class TaskbarNotifier : Form
	{
		...
		public TaskbarNotifier()
		{
			...
			TitleRectangle = new Rectangle(40, 6, 95, 60);
			ContentRectangle = new Rectangle(8, 41, 133, 68);
		...
		public void Show(string strTitle, string strContent, int nTimeToShow, int nTimeToStay, int nTimeToHide)
		{
            ...
            var height = BackgroundBitmap == null ? 20 : BackgroundBitmap.Height;
			// We calculate the pixel increment and the timer value for the showing animation
			int nEvents;
			if (nTimeToShow > 10)
			{
				nEvents = Math.Min((nTimeToShow / 10), height);
				nShowEvents = nTimeToShow / nEvents;
				nIncrementShow = height / nEvents;
			}
			else
			{
				nShowEvents = 10;
				nIncrementShow = height;
			}

			// We calculate the pixel increment and the timer value for the hiding animation
			if( nTimeToHide > 10)
			{
				nEvents = Math.Min((nTimeToHide / 10), height);
				nHideEvents = nTimeToHide / nEvents;
				nIncrementHide = height / nEvents;
			}
			else
			{
				nHideEvents = 10;
				nIncrementHide = height;
			}

		    var width = BackgroundBitmap == null ? 50 : BackgroundBitmap.Width;
			switch (taskbarState)
			{
				case TaskbarStates.hidden:
					taskbarState = TaskbarStates.appearing;
                    SetBounds(WorkAreaRectangle.Right - width - 17, WorkAreaRectangle.Bottom - 1, width, 0);
				...
				case TaskbarStates.disappearing:
					...
					SetBounds(WorkAreaRectangle.Right-width-17, WorkAreaRectangle.Bottom-height-1, width, height);
					timer.Interval = nVisibleEvents;
					timer.Start();
					Refresh();
					break;
		...
		{
			SetBackgroundBitmap("skin.gif", Color.FromArgb(255, 0, 255));
		}

		public void SetCloseBitmap()
		{
            SetCloseBitmap("close.gif", Color.FromArgb(255, 0, 255), new Point(127, 8));
		}
		/// <summary>
		...
		public void SetCloseBitmap(string strFilename, Color transparencyColor, Point position)
		{
			...
			//CloseBitmap = new Bitmap(getImage(strFilename));
